...
getDensity(char *file, int num_points, int start_point, vector<BND> bndr, int num_divider, int dim) {
    ...
    //Initialise density vector
    vector<int> density;
    density.reserve(num_cells);
    ...
    double bn; //biggest number
    int bp; //position of bn
    for (int i = 0; i < num_points; i++) {
        bn = FM[i][0];// biggest number
        bp = 0;// biggest number position
        for (int j = 0; j < num_cells; j++) {
            tDen = 0.0;
            ...
            //Find the highest membership to the cluster
            if (bn < FM[i][j]) {
                bp = j;
                bn = FM[i][j];
            }
        }
        density.at(bp)++;//count up appropriate cluster
    }
    return density;
}
...
int main() {
    ...
    _div = 6; // divider of each axis(More divider, more accuracy but slower speed)

    clock_t tStart = clock();
    vector<int> dcm = getDensity(_file, _num_points, _start_point_DCM, bndr, _div, _dim);
    vector<int> wcm;
